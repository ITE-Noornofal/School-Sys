...
class StudentController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        |--------------------------------------------------------------------------
        |
        | users أصبح جدول الحسابات الموحد.
        |
        */
        ...
        // التأكد أن User مرتبط فعلاً بطالب
        if (!$user->student) {
            return response()->json([
                'status' => 'error',
    ...
    public function logout(Request $request)
    {
        ...
        return response()->json([
            'message' => 'you logged out sucssesfully goodbye'
        ]);
    }
    ...
    public function saveProfile(Request $request)
    {
        ...
        // ================================================================
        // تحديث اسم الطالب في جدول students
        // ================================================================
        ...
        return response()->json([
            'message' =>
                'The profile information has been successfully saved',
    ...
    public function availableTrips()
    {
        ...
        if (!$student) {
            return response()->json([
                'message' =>
                    'ℹ️ No student linked to this user.',
            ], 404);
        }

        // جلب الرحلات المؤكدة للطالب من جدول pivot
        $confirmedTrips = $student
            ->schoolTrips()
            ->wherePivot(
                'confirmation_status',
                'confirmed'
        ...
        if ($confirmedTrips->isNotEmpty()) {
            ...
            return response()->json([
                'message' =>
                    'ℹ️ You already have a confirmed trip.',
        ...
        if ($trips->isEmpty()) {
            ...
            return response()->json([
                'message' =>
                    'ℹ️ No available trips for your class at the moment.',
            ], 200);
        }


        $availableTrips = $trips->map(
            function ($trip) {
        ...
        return response()->json([
            'message' =>
                '✅ Available trips for your class.',
    ...
    public function storeByStudent(Request $request)
    {
        ...
        if (!$student) {
            return response()->json([
                'message' =>
                    '❌ You are not linked to a student record.'
            ], 403);
        }


        $isAnonymous =
            $request->input('is_anonymous', false);

        $studentIdHash =
            hash('sha256', $student->id);


        if ($isAnonymous) {

            $anonymousCount = Complaint::where(
                'student_id_hash',
                $studentIdHash
            )
                ->where(
            ...
            if ($anonymousCount >= 3) {
                ...
                return response()->json([
                    'message' =>
                        '❌ You have exceeded the allowed number of anonymous complaints this month.'
                ], 403);
            }
        }
        ...
        if ($isAnonymous) {
            ...
            $complaintData['student_id'] =
                null;

            $complaintData['student_id_enc'] =
                Crypt::encryptString(
                    $student->id
                );

            $complaintData['student_id_hash'] =
                $studentIdHash;
